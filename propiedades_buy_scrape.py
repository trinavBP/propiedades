...
while True:
    # Keep reducing the maxInt value by half until we find a valid value
    try:
...
BUY_URL_TEMPLATE = 'https://propiedades.com/df/venta?pagina={page_num}'
MAX_RETRIES = 3
RETRY_BACKOFF = 5
scraped_urls = set()
...
def fetch_page(page_num):
    ...
    while retries < MAX_RETRIES:
        try:
            scraperapi_url = f'{BASE_URL}?api_key={SCRAPER_API_KEY}&url={BUY_URL_TEMPLATE.format(page_num=page_num)}'
...
def extract_property_data(page_content):
    ...
    price_pattern = re.compile(r'"sale_price_real":(\d+)')
    ...
    prices = [int(x) for x in price_pattern.findall(page_content)]
    ...
    for i in range(total_properties):
        ...
        property_data = {
            ...
            'buy_price_mxn': prices[i] if i < len(prices) else 0,
            'buy_price_usd': prices[i] / MXN_TO_USD_CONVERSION_RATE if i < len(prices) else 0,
            'rent_price_mxn': 0,
            'rent_price_usd': 0,
    ...
    prices_mxn = np.array([prop['buy_price_mxn'] for prop in properties if prop['buy_price_mxn']])
    ...
    if len(prices_mxn) > 0 and len(sizes_m2) > 0:
        ...
        for prop in properties:
            prop['standardized_price'] = (prop['buy_price_mxn'] - mean_price) / std_price if prop['buy_price_mxn'] else None
...
def scrape_properties(max_pages=None, test_mode=False):
    ...
    csv_filename = 'propiedades_data_buy.csv'
